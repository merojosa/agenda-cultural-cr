<script lang="ts">
	...
	console.log('BREAKPOINT', data.activity.datetime);
</script>

<article class="flex flex-col gap-8">
	<div class="grid grid-cols-[40%_60%] gap-12">
		<img class="w-full" src={data.activity.imageUrl} alt={`Portada de ${data.activity.title}`} />

		<div class="flex flex-col gap-8">
			<h1 class="text-5xl font-extrabold leading-tight">{data.activity.title}</h1>
			<ul class="flex flex-col gap-8">
				...
				<li class="flex flex-row gap-2 items-center">
					...
					<Button
						class="text-xl p-0"
						variant="link"
						href={data.activity.activityUrl}
						target="_blank"
					>
						Sitio oficial
					</Button>
