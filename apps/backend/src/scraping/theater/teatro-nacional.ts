...
function getCurrentMonth(page: Page) {
  return page.evaluate((spanishMonths) => {
    const currentMonthElement = document.querySelector(
      "#calMonths > .current > span"
    );
    const monthText = currentMonthElement?.textContent?.toLocaleLowerCase();
    const currentMonthValue =
      spanishMonths[monthText || ""] || DateTime.local().month;

    return currentMonthValue;
  }, spanishMonths);
}

async function getTeatroNacionalBasicData(
  page: Page,
  year: number,
  month: number
) {
...
async function getDescriptionAndSource(
  ...
  const xPathElementsHandle = await page.$x(
    ...
    )})]]/following-sibling::h3[text()[contains(., ${escapeXpathString(
      titlePlay
    )})]]`
  );
  ...
  const descriptionParagraphs = await page.evaluate(() => {
    const pElements = document.querySelectorAll(
      "section > .generalWrap > .calCol2 > hr ~ p"
    );
...
export async function getTeatroNacionalData(page: Page) {
  ...
  const teatroNacionalDays = await getTeatroNacionalBasicData(
    page,
    currentYear,
    currentMonth
  );
